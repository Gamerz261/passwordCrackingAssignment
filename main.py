...
def main(argv):
    #print(sys.argv)
    variable = str(sys.argv[len(sys.argv)-1]).encode('utf-8') # Takes in arguments from the command line
...
def decrypt(variable):
    ...
    if '-f' in sys.argv:
        ...
        variable = str(input("Input the Bcrypt hash to be decrypted: ")).rstrip()
        print(variable)
...
def encrypt(variable):
    ...
    if '-m' in sys.argv:
        ...
        print(green + "BCrypt Encrypted: " + white + (str(runner.encrypt()).rstrip())[2:-1])
    else:
